...
let processedComplaintId: number;
// 결재 상신 후 생성된 결재 ID
let createdApprovalId: number;

describe('결재 서비스 및 API', () => {
  beforeAll(async () => {
  ...
  describe('POST /api/approvals — 결재 상신', () => {
    it('담당자가 결재를 상신하면 201 상태와 결재 정보를 반환한다', async () => {
      ...
      expect(res.body.title).toBe('건축허가 결재 요청');
      expect(res.body.content).toBe('건축허가 민원 처리 결과를 결재 요청합니다');
      expect(res.body.requesterId).toBe(officerId);
      expect(res.body.approverId).toBe(approverId);
      expect(res.body.status).toBe('PENDING');
      expect(res.body.requesterName).toBe('결재테스트담당자');
      ...
      expect(complaint?.status).toBe('PENDING_APPROVAL');
    });

    it('결재 제목이 누락되면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .post('/api/approvals')
        .set('Authorization', `Bearer ${officerToken}`)
        .send({
    ...
    it('결재 내용이 빈 문자열이면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .post('/api/approvals')
        .set('Authorization', `Bearer ${officerToken}`)
        .send({
    ...
    it('승인권자가 누락되면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .post('/api/approvals')
        .set('Authorization', `Bearer ${officerToken}`)
        .send({
    ...
    it('유효하지 않은 승인권자 ID로 상신하면 400 오류를 반환한다', async () => {
    ...
    it('민원_신청인이 결재를 상신하면 403 오류를 반환한다', async () => {
      const res = await request(app)
        .post('/api/approvals')
    ...
    it('존재하지 않는 민원에 결재를 상신하면 404 오류를 반환한다', async () => {
      const res = await request(app)
        .post('/api/approvals')
        .set('Authorization', `Bearer ${officerToken}`)
        .send({
    ...
    it('RECEIVED 상태 민원에 결재를 상신하면 409 오류를 반환한다', async () => {
      const c = await prisma.complaint.create({
        data: {
  ...
  describe('GET /api/approvals — 결재 목록 조회', () => {
    it('승인권자가 결재 목록을 조회하면 본인에게 배정된 결재 목록을 반환한다', async () => {
      const res = await request(app)
        .get('/api/approvals')
      ...
      for (const item of res.body.items) {
        expect(item.approverId).toBe(approverId);
        expect(item.requesterName).toBeDefined();
      }
    });

    it('담당자가 결재 목록을 조회하면 403 오류를 반환한다', async () => {
      const res = await request(app)
        .get('/api/approvals')
        .set('Authorization', `Bearer ${officerToken}`);
    ...
    it('민원_신청인이 결재 목록을 조회하면 403 오류를 반환한다', async () => {
      const res = await request(app)
        .get('/api/approvals')
        .set('Authorization', `Bearer ${applicantToken}`);
    ...
    it('인증 없이 결재 목록을 조회하면 401 오류를 반환한다', async () => {
      const res = await request(app)
        .get('/api/approvals');
  ...
  describe('GET /api/approvals/:id — 결재 상세 조회', () => {
    it('승인권자가 결재 상세를 조회하면 민원 처리 내역, 검토 의견, 통보 이력이 포함된다', async () => {
      const res = await request(app)
        .get(`/api/approvals/${createdApprovalId}`)
        .set('Authorization', `Bearer ${approverToken}`);

      expect(res.status).toBe(200);
      expect(res.body.id).toBe(createdApprovalId);
      expect(res.body.title).toBe('건축허가 결재 요청');
      expect(res.body.complaint).toBeDefined();
      expect(res.body.complaint.processType).toBe('APPROVE');
      expect(res.body.complaint.processReason).toBe('승인 처리합니다');
      expect(res.body.reviewComment).toBe('검토 완료');
      expect(res.body.notifications).toBeDefined();
      expect(Array.isArray(res.body.notifications)).toBe(true);
      expect(res.body.notifications.length).toBeGreaterThanOrEqual(1);
    });

    it('담당자가 본인이 상신한 결재를 조회할 수 있다', async () => {
      const res = await request(app)
        .get(`/api/approvals/${createdApprovalId}`)
        .set('Authorization', `Bearer ${officerToken}`);
    ...
    it('민원_신청인이 결재를 조회하면 403 오류를 반환한다', async () => {
      const res = await request(app)
        .get(`/api/approvals/${createdApprovalId}`)
        .set('Authorization', `Bearer ${applicantToken}`);
    ...
    it('존재하지 않는 결재 ID로 조회하면 404 오류를 반환한다', async () => {
      const res = await request(app)
    ...
    it('유효하지 않은 결재 ID로 조회하면 400 오류를 반환한다', async () => {
      const res = await request(app)
  ...
  describe('PUT /api/approvals/:id/approve — 결재 승인', () => {
    // 승인 테스트용 결재 ID
    let approveTestApprovalId: number;
    let approveTestComplaintId: number;
    ...
    it('승인권자가 승인 사유와 함께 결재를 승인하면 성공한다', async () => {
      const res = await request(app)
      ...
      expect(res.body.approvalReason).toBe('검토 결과 적합하여 승인합니다');
      ...
      expect(complaint?.status).toBe('APPROVED');
    });

    it('승인 사유가 누락되면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${approveTestApprovalId}/approve`)
        .set('Authorization', `Bearer ${approverToken}`)
        .send({});
    ...
    it('승인 사유가 빈 문자열이면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${approveTestApprovalId}/approve`)
        .set('Authorization', `Bearer ${approverToken}`)
        .send({ reason: '  ' });
    ...
    it('이미 승인된 결재를 다시 승인하면 409 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${approveTestApprovalId}/approve`)
        .set('Authorization', `Bearer ${approverToken}`)
        .send({ reason: '재승인 시도' });
    ...
    it('승인권자가 아닌 사용자가 승인하면 403 오류를 반환한다', async () => {
      // 새 PENDING 결재 생성
      const c = await prisma.complaint.create({
        data: {
    ...
    it('존재하지 않는 결재를 승인하면 404 오류를 반환한다', async () => {
      const res = await request(app)
  ...
  describe('PUT /api/approvals/:id/reject — 결재 반려', () => {
    // 반려 테스트용 결재 ID
    let rejectTestApprovalId: number;
    let rejectTestComplaintId: number;
    ...
    it('승인권자가 반려 사유와 후속 조치 사항을 입력하여 반려하면 성공한다', async () => {
      const res = await request(app)
      ...
      expect(res.body.rejectionReason).toBe('서류가 미비하여 반려합니다');
      expect(res.body.followUpAction).toBe('추가 서류를 보완하여 재상신해주세요');
      expect(res.body.decidedAt).toBeDefined();
    ...
    it('반려 사유가 누락되면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${approverToken}`)
    ...
    it('후속 조치 사항이 누락되면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${approverToken}`)
        .send({ reason: '반려 사유' });
    ...
    it('반려 사유가 빈 문자열이면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${approverToken}`)
    ...
    it('후속 조치 사항이 빈 문자열이면 400 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${approverToken}`)
    ...
    it('이미 반려된 결재를 다시 반려하면 409 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${approverToken}`)
    ...
    it('승인권자가 아닌 사용자가 반려하면 403 오류를 반환한다', async () => {
      const res = await request(app)
        .put(`/api/approvals/${rejectTestApprovalId}/reject`)
        .set('Authorization', `Bearer ${officerToken}`)
    ...
    it('존재하지 않는 결재를 반려하면 404 오류를 반환한다', async () => {
      const res = await request(app)
