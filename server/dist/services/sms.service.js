...
exports.clearVerificationSessions = clearVerificationSessions;
 ...
 * 인증 세션 저장소 초기화 (테스트용)
 */
function clearVerificationSessions() {
 ...
 */
async function sendVerificationCode(receiptNumber) {
    // 접수번호로 민원 조회
    const complaint = await prisma.complaint.findUnique({
        where: { receiptNumber },
    });
    if (!complaint) {
        throw (0, errors_1.notFoundError)('해당 접수번호의 민원을 찾을 수 없습니다');
    }
    // 인증 세션 생성
 ...
 * @returns 민원 상세 정보
 */
async function confirmVerification(verificationId, code) {
    // 인증 세션 조회
    const session = verificationSessions.get(verificationId);
    if (!session) {
        throw (0, errors_1.notFoundError)('유효하지 않은 인증 세션입니다');
    }
    // 인증 코드 확인
    if (session.code !== code) {
        throw (0, errors_1.unauthorizedError)('본인 확인에 실패하였습니다');
    }
