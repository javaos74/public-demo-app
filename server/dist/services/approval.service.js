 ...
 * @returns 생성된 결재 정보
 */
async function createApproval(complaintId, title, content, requesterId, approverId) {
    // 필수 항목 검증
    if (!title || String(title).trim() === '') {
        throw (0, errors_1.validationError)('결재 제목을 입력해주세요');
    }
    if (!content || String(content).trim() === '') {
        throw (0, errors_1.validationError)('결재 내용을 입력해주세요');
    }
    if (!approverId) {
        throw (0, errors_1.validationError)('승인권자를 선택해주세요');
    }
    // 민원 존재 여부 및 상태 확인
    ...
    if (complaint.status !== 'PROCESSED') {
        throw (0, errors_1.invalidStatusTransitionError)(`현재 상태(${complaint.status})에서 PENDING_APPROVAL(으)로 변경할 수 없습니다`);
    }
    ...
    if (!approver || approver.role !== 'APPROVER') {
        throw (0, errors_1.validationError)('유효하지 않은 승인권자입니다');
    }
    ...
    if (existingApproval) {
        throw (0, errors_1.validationError)('이미 결재가 상신된 민원입니다');
    }
    ...
    await (0, complaint_service_1.updateComplaintStatus)(complaintId, 'PENDING_APPROVAL');
    return {
        ...approval,
 ...
 */
async function getApprovals(approverId, page, limit) {
 ...
 * @returns 업데이트된 결재 정보
 */
async function approveApproval(id, reason, user) {
    // 승인 사유 필수 검증
    if (!reason || String(reason).trim() === '') {
        throw (0, errors_1.validationError)('승인 사유를 입력해주세요');
    }
    // 결재 존재 여부 확인
    const approval = await prisma.approval.findUnique({
        where: { id },
    ...
    if (approval.status !== 'PENDING') {
        throw (0, errors_1.invalidStatusTransitionError)(`현재 상태(${approval.status})에서 APPROVED(으)로 변경할 수 없습니다`);
    }
    // 승인권자 본인만 승인 가능
    if (approval.approverId !== user.id) {
        throw (0, errors_1.forbiddenError)('본인에게 배정된 결재만 승인할 수 있습니다');
    }
    ...
    // 민원 상태를 APPROVED로 변경
    await (0, complaint_service_1.updateComplaintStatus)(approval.complaintId, 'APPROVED');
    return {
        ...updated,
 ...
 * @returns 업데이트된 결재 정보
 */
async function rejectApproval(id, reason, followUpAction, user) {
    // 반려 사유 필수 검증
    if (!reason || String(reason).trim() === '') {
        throw (0, errors_1.validationError)('반려 사유를 입력해주세요');
    }
    // 후속 조치 사항 필수 검증
    if (!followUpAction || String(followUpAction).trim() === '') {
        throw (0, errors_1.validationError)('후속 조치 사항을 입력해주세요');
    }
    // 결재 존재 여부 확인
    const approval = await prisma.approval.findUnique({
        where: { id },
    ...
    // PENDING 상태에서만 반려 가능
    if (approval.status !== 'PENDING') {
        throw (0, errors_1.invalidStatusTransitionError)(`현재 상태(${approval.status})에서 REJECTED(으)로 변경할 수 없습니다`);
    }
    // 승인권자 본인만 반려 가능
    if (approval.approverId !== user.id) {
        throw (0, errors_1.forbiddenError)('본인에게 배정된 결재만 반려할 수 있습니다');
    }
    ...
    // 민원 상태를 REJECTED로 변경
    await (0, complaint_service_1.updateComplaintStatus)(approval.complaintId, 'REJECTED');
    return {
        ...updated,
        requesterName: updated.requester.name,
        reviewComment: updated.complaint.reviewComment,
