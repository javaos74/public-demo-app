...
exports.reviewComplaint = reviewComplaint;
exports.getApplicantStatus = getApplicantStatus;
exports.processComplaint = processComplaint;
exports.createNotification = createNotification;
exports.getNotifications = getNotifications;
const client_1 = require("@prisma/client");
const receipt_number_1 = require("../utils/receipt-number");
 ...
 * @returns 생성된 민원 정보
 */
async function createComplaint(input, applicantId) {
    // 필수 항목 검증
    const { type, title, content, contactPhone } = input;
    if (!type || !title || !content || !contactPhone) {
        throw (0, errors_1.validationError)('필수 입력 항목이 누락되었습니다');
    }
    // 빈 문자열 검증
    if (String(type).trim() === '' ||
        String(title).trim() === '' ||
    ...
    if (isNaN(typeId)) {
        throw (0, errors_1.validationError)('민원 유형 ID는 숫자여야 합니다');
    }
    // 민원 유형 존재 여부 확인
    ...
    if (!complaintType) {
        throw (0, errors_1.validationError)('존재하지 않는 민원 유형입니다');
    }
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        const receiptNumber = (0, receipt_number_1.generateReceiptNumber)(now, todayCount + 1 + attempt);
        try {
    ...
    throw (0, errors_1.validationError)('접수번호 생성에 실패했습니다. 잠시 후 다시 시도해주세요.');
}
/**
 ...
 */
async function getComplaints(query, user) {
...
async function getComplaintById(id, user) {
    ...
    if (!complaint) {
        throw (0, errors_1.notFoundError)('해당 민원을 찾을 수 없습니다');
    }
    // 민원_신청인은 본인 민원만 조회 가능
    if (user.role === 'APPLICANT' && complaint.applicantId !== user.id) {
        throw (0, errors_1.forbiddenError)('본인이 접수한 민원만 조회할 수 있습니다');
    }
 ...
 */
async function saveDocuments(complaintId, files) {
 ...
 * @returns 문서 정보
 */
async function getDocumentById(docId, complaintId) {
    const document = await prisma.document.findUnique({
        where: { id: docId },
    });
    if (!document) {
        throw (0, errors_1.notFoundError)('해당 문서를 찾을 수 없습니다');
    }
    // 문서가 해당 민원에 속하는지 확인
    if (document.complaintId !== complaintId) {
        throw (0, errors_1.notFoundError)('해당 민원에 속하지 않는 문서입니다');
    }
    return document;
}
/**
...
async function updateComplaintStatus(id, newStatus) {
    ...
    if (!allowedTransitions || !allowedTransitions.includes(newStatus)) {
        throw (0, errors_1.invalidStatusTransitionError)(`현재 상태(${currentStatus})에서 ${newStatus}(으)로 변경할 수 없습니다`);
    }
    // 상태 업데이트
    const updated = await prisma.complaint.update({
        where: { id },
 ...
 * @returns 업데이트된 민원 정보
 */
async function reviewComplaint(id, reviewComment, user) {
    // 검토 의견 필수 검증
    if (!reviewComment || String(reviewComment).trim() === '') {
        throw (0, errors_1.validationError)('검토 의견을 입력해주세요');
    }
    // 민원 존재 여부 확인
    const complaint = await prisma.complaint.findUnique({
        where: { id },
    ...
    if (complaint.status !== 'REVIEWING') {
        throw (0, errors_1.invalidStatusTransitionError)('검토중 상태의 민원만 검토 의견을 저장할 수 있습니다');
    }
    // 검토 의견 업데이트
    const updated = await prisma.complaint.update({
        where: { id },
 ...
 * @returns 민원인 현황 데이터
 */
async function getApplicantStatus(complaintId, user) {
    // 민원 존재 여부 확인
    const complaint = await prisma.complaint.findUnique({
    ...
    if (!mockStatus) {
        throw (0, errors_1.notFoundError)('해당 민원인의 현황 데이터가 없습니다');
    }
    return {
        id: mockStatus.id,
 ...
 * @returns 업데이트된 민원 정보
 */
async function processComplaint(id, processType, processReason, officerId) {
    // 처리 유형 유효성 검증
    const validProcessTypes = ['APPROVE', 'REJECT', 'HOLD', 'TRANSFER'];
    if (!processType || !validProcessTypes.includes(processType)) {
        throw (0, errors_1.validationError)('유효하지 않은 처리 유형입니다 (APPROVE/REJECT/HOLD/TRANSFER)');
    }
    // 처리 사유 필수 검증
    if (!processReason || String(processReason).trim() === '') {
        throw (0, errors_1.validationError)('처리 사유를 입력해주세요');
    }
    // 민원 존재 여부 확인
    const complaint = await prisma.complaint.findUnique({
        where: { id },
    ...
    if (currentStatus !== 'REVIEWING') {
        throw (0, errors_1.invalidStatusTransitionError)(`현재 상태(${currentStatus})에서 PROCESSED(으)로 변경할 수 없습니다`);
    }
    // 민원 처리 정보 업데이트
    const updated = await prisma.complaint.update({
        where: { id },
 ...
 * @returns 생성된 통보 레코드
 */
async function createNotification(complaintId, targetAgency, notificationContent) {
    // 필수 항목 검증
    if (!targetAgency || String(targetAgency).trim() === '') {
        throw (0, errors_1.validationError)('통보 대상 기관을 입력해주세요');
    }
    if (!notificationContent || String(notificationContent).trim() === '') {
        throw (0, errors_1.validationError)('통보 내용을 입력해주세요');
    }
    // 민원 존재 여부 확인
    const complaint = await prisma.complaint.findUnique({
        where: { id: complaintId },
